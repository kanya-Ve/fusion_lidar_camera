#include "fusion_lidar_camera/direct_node.hpp"
#include <sstream>
#include <iomanip>
#include <chrono>
#include <cmath>
...
namespace fusion_lidar_camera
{
    ...
    FusionLidarCamera::FusionLidarCamera() : Node("fusion_lidar_camera")
    {
        ...
        debug_timer_ = this->create_wall_timer(
            ...
            std::bind(&FusionLidarCamera::debugCallback, this));
    ...
    void FusionLidarCamera::debugCallback()
    {
        ...
        cv::imshow("Camera", _frame);

        // Exit loop if 'q' is pressed
        if (cv::waitKey(1) == 'q')
        {
            RCLCPP_INFO(this->get_logger(), "Exiting camera display loop.");
    ...
    {
        if (!_pointcloud || !_image)
        {
            RCLCPP_WARN(this->get_logger(), "Waiting for all inputs: point cloud and image");
            return;
        }
        ...
        RCLCPP_INFO(this->get_logger(), "Using PointCloud2 timestamp: %f",
                    _pointcloud->header.stamp.sec + _pointcloud->header.stamp.nanosec * 1e-9);

        // Call the main processing function
        callback(_pointcloud, _image);
    };
                                     ...
                                     const sensor_msgs::msg::Image::ConstSharedPtr &image_msg)
    {
        ...
        try
        {
            ...
            classifyPointsByDetections(cloud, image_msg, colored_cloud, projected_image);

            // Publish results and save PCD files (描画・保存処理)
            publishAndSaveResults(colored_cloud, projected_image,
                                  cloud_msg, image_msg);
        }
    ...
    void FusionLidarCamera::classifyPointsByDetections(
...
int main(int argc, char const *argv[])
{
    ...
    auto node = std::make_shared<fusion_lidar_camera::FusionLidarCamera>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
